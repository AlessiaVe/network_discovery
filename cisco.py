...
class Cisco(SNMP):
    ...
    def interfaces_MAC(self):
        ...
        if self._interfaces_MAC is None:
            ...
            for i in self._value_to_retrieve():
                ...
                # toname_ip obtain something as 00:11:22:22:33:44:55
    ...
    def get_neighbours(self):
        """
        returns an ordered dict with the neighbours of the device
        """
        if self._neighbours is None:
            results = []
            starting = "1.0.8802.1.1.2.1.4.1.1.9."
            names = self.next(starting)[3]
            oid = ""
            name = ""

            for i in range(len(names)):
                oid = str(names[i][0][0])
                name = str(names[i][0][1])
    ...
    def get_neighbours_ip(self):
        if self._neighbours is None:
            results = []
            starting = "1.3.6.1.2.1.4.22.1.2."
            ips = self.next(starting)[3]
            ip = ""
    ...
    _neighbours_names = None

    def get_neighbours_names(self):
        """
        returns an ordered dict with the neighbours of the device
        """
        if self._neighbours_names is None:
            results = []
            starting = "1.0.8802.1.1.2.1.4.1.1.9."
            names = self.next(starting)[3]
            name = ""
            ...
            self._neighbours_names = results

        return self._neighbours_names
